...
固定高度的实时思考是自动更新的预览，不再作为第二层滚动阅读区。
鼠标位于思考文字上时，滚轮和触控板事件交给外层正文；“完整思考”弹窗仍可独立滚动。
这避免整页已暂停跟随、但滚动量仍被内层思考窗口消耗的情况。

回归：运行 `scripts/build-reading-preview.sh`，选择“只有思考”，打开“模拟流式回复”。
在思考文字上向上滚动，应立即进入上方历史且停留位置不被后续输出拉回；
点击“返回最新回复”后预览继续显示最新思考；展开完整思考，确认弹窗可独立滚动。

隔离实测（同一正文组件，20 轮历史，每 100 ms 追加思考，鼠标位于预览文字上）：
相同的向上 376 pt 滚动，修复前整页位置为 9470 → 9470，修复后为 9470 → 9094。
普通流式正文的向上滚动与暂停跟随亦已单独检查。此结果不替代正式远端会话的验收。
