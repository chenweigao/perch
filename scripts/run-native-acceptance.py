...
app = root / "build/Perch Acceptance.app"
...
manifest.update(binary_sha256=hashlib.sha256(binary.read_bytes()).hexdigest(), capture=args.capture,
                mode="frames" if args.capture == "frames" else "all")
...
if args.capture != "none":
    ...
    for key in ["PERCH_ACCEPTANCE_MODE", "PERCH_ACCEPTANCE_RESULTS", "PERCH_ACCEPTANCE_STALL", "PERCH_ACCEPTANCE_KEEP_OPEN"]:
...
with (out / "process.log").open("w") as log:
    ...
    try:
        manifest["exit_code"] = process.wait(timeout=95 if args.capture != "none" else 60)
