...
The review branch also incorporates main at `77b7308`; the sidebar conflict was resolved by retaining its larger filter target/alignment and the translated filter label. The integrated Release app passed WorkbenchChecks, ConnectionChecks, catalog coverage and signature verification. The additional ComposerChecks process was terminated with SIGKILL twice (including after ad-hoc re-signing); its cause is unconfirmed and this check is not counted as passing. No live agent prompts were sent for acceptance.
