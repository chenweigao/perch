...
ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 "$remote_host" 'mkdir -p ~/.local/share/agent-workbench/native; chmod 700 ~/.local/share/agent-workbench/native'
scp -q -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 remote/native-agent-service.py remote/qoder-worker.mjs remote/package.json "$remote_host:.local/share/agent-workbench/native/"
if [[ "$provider" == "all" || "$provider" == "qoder" ]]; then
  ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 "$remote_host" 'cd ~/.local/share/agent-workbench/native && npm install --ignore-scripts'
fi
if [ "$with_dsh" = "1" ]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 "$remote_host" 'set -e; cd ~/.local/share/agent-workbench/native &&
