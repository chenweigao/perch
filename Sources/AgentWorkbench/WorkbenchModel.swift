...
final class WorkbenchModel: ObservableObject {
    ...
    func select(_ identity: String) {
        ...
        if let reference = selectedReference {
            ...
            if [.omp, .qoder, .dsh].contains(reference.kind) { native.select(reference.terminalID) }
    ...
    func markReviewed(_ item: WorkspaceSession) {
        ...
        if [.omp, .qoder, .dsh].contains(item.reference.kind), let session = native.sessions.first(where: { $0.id == item.reference.terminalID }) {
    ...
    func archiveSubject(_ item: WorkspaceSession) -> ArchiveSubject {
        ...
        if [.omp, .qoder, .dsh].contains(item.reference.kind) {
    ...
    private func archiveRequest(_ reference: SessionReference, archived: Bool = true) async throws {
        ...
        else if [.omp, .qoder, .dsh].contains(reference.kind) {
    ...
    func canArchive(_ item: WorkspaceSession) -> Bool {
        ...
        if [.omp, .qoder, .dsh].contains(item.reference.kind) { return native.online && native.sessions.first(where: { $0.id == item.reference.terminalID })?.busy == false }
    ...
    func setArchived(_ item: WorkspaceSession, archived: Bool) {
        ...
        Task {
            ...
            do {
                ...
                else if [.omp, .qoder, .dsh].contains(item.reference.kind) { try await native.action(item.reference.terminalID, "archive", ["archived": .bool(archived)]) }
    ...
    func deleteConfirmed(_ item: WorkspaceSession) {
        ...
        Task {
            ...
            do {
                ...
                else if [.omp, .qoder, .dsh].contains(item.reference.kind) { try await native.action(item.reference.terminalID, "delete") }
