...
func checkWorkflow() throws {
    ...
    let hits = ConversationSearch.hits(in: messages, query: "match", running: false)
    precondition(hits.count == 2 && hits[1].occurrence == 1)
    precondition(ConversationSearch.hits(in: messages, query: "Match one", running: false).count == 1, "Search uses rendered body text")
    precondition(hits[0].entryID == ConversationTimelineEntry.make(messages)[0].id)
    precondition(ConversationSearch.hits(in: messages, query: "", running: false).isEmpty)
