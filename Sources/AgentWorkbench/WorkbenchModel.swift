...
final class WorkbenchModel: ObservableObject {
    ...
    func select(_ identity: String) {
        ...
        if let reference = selectedReference {
            ...
            if [.omp, .qoder].contains(reference.kind) { native.select(reference.terminalID) }
    ...
    var canQuoteSelection: Bool {
        ...
        return reference.kind == .kimi ? kimi.online : [.omp, .qoder].contains(reference.kind) && native.online
    }
    ...
    func setArchived(_ item: WorkspaceSession, archived: Bool) {
        ...
        Task {
            ...
            do {
                ...
                else if [.omp, .qoder].contains(item.reference.kind) { try await native.action(item.reference.terminalID, "archive", ["archived": .bool(archived)]) }
    ...
    func deleteConfirmed(_ item: WorkspaceSession) {
        ...
        Task {
            ...
            do {
                ...
                else if [.omp, .qoder].contains(item.reference.kind) { try await native.action(item.reference.terminalID, "delete") }
